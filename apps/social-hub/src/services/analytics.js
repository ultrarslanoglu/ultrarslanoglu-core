...
class AnalyticsService {
    ...
    try {
      if (!config.features.xEnabled) {
        return {
          platform: 'x',
          disabled: true,
          message: 'X integration disabled (paid access required)',
          summary: this.getEmptySummary()
        };
      }
  ...
  async getAllStats(userId) {
    try {
      ...
      if (config.features.xEnabled && connectedPlatforms.includes('x')) {
        promises.push(
          this.getXStats(userId)
            .then(result => { stats.x = result; })
            .catch(error => { stats.x = { error: error.message }; })
        );
      } else if (!config.features.xEnabled) {
        stats.x = {
          disabled: true,
          message: 'X integration disabled (paid access required)',
          summary: this.getEmptySummary()
  ...
  createOverallSummary(stats) {
    ...
    for (const [platform, data] of Object.entries(stats)) {
      if (data.error || data.disabled) continue;
