...
class VideoUploadRequest(BaseModel):
    ...
    description: Optional[str] = Field(
        None, max_length=1000, description="Video description"
    )
...
class TranscodeRequest(BaseModel):
    ...
    target_format: str = Field(
        ..., pattern="^(mp4|webm|mkv|avi)$", description="Target format"
    )
...
class MetricRequest(BaseModel):
    ...
    platform: str = Field(
        ..., description="Platform name (instagram, tiktok, youtube, etc)"
    )
